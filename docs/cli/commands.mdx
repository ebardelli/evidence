...
Behind an authenticating reverse proxy (see [Authentication & SSO](/self-host/authentication)), set `EVIDENCE_AUTH_PROXY_EMAIL_HEADER` (and optionally `EVIDENCE_AUTH_PROXY_NAME_HEADER`) to the proxy's forwarded-identity headers to show the signed-in user in the sidebar. This is cosmetic only — it does not authenticate anyone.
